...
Following\pageoriginale a discussion of the basic \textit{Ahlfors lemma} and its variants in \S \ref{art4-sec7}(a), we have given Borel's proof of the quasi-unipotence of the \textit{Picard-Lefschetz transformation} in \ref{art4-sec7}(b); this should illustrate in a simple fashion the power of the method.
...
We conclude with an appendix, reviewing the current status of the problems and conjectures contained in the report \cite{art4-key21} of the first author.
...
\begin{coro}\label{art4-coro5.3}
Let $\bar{X}$ be a smooth compactification (\cf \S \ref{art4-sec5}(a) below) of $X$. Then the image of $H^n (\bar{X}) \to H^n (X)$ is $W_n \{H^n (X)\}$.
\end{coro}
...
\begin{proof}
According to Deligne's degeneration theorem discussed in \S \ref{art4-sec2}(a), the image of 
$$
...
\begin{theorem}[\textsc{nilpotent orobit theorem}]\label{art4-eq9.7}
...
Let\pageoriginale us first look at the simplest possible case, when $D$ is the ordinary upper half plane $U$, $\check{D}$ the Riemann sphere $\bP^1 = \bC \cup \{\infty\}$, and $G_\bR$ the group $SL (2, \bR)$. The Lie algebra $\fs\fl (2, \bR)$ contains exactly two conjugacy classes of nonzero nilpotent elements, namely those of
$$
\begin{pmatrix}
0 & 1 \\
0 & 0
...
\begin{theorem}[\textsc{$SL_2$-orbit theorem}]\label{art4-thm9.19} 
...
We now introduce the three $\fg_0$-valued functions 
\begin{equation}
...
\label{art4-eq10.3}
\end{equation}
Since $N$ is defined over $\bQ$, so is the filtration. The vector spaces $P^m (V_t, \bC)$ are the fibres of a flat bundle $\bP^m \to \Delta^\ast$ (\cf 3(a)), and with respect to the flat structure, $N$ becomes independent of $t$. Hence \eqref{art4-eq10.2} defines a filtration of $\bP^m$ by flat subbundles
\begin{equation}
...
The total space $\bar{X}$ has the central fibre $V_0$ as a strong retract (\cf \cite{art4-key8}), so that $H^\ast (\bar{X}) \cong H^\ast (V_0)$. Taking into account Poincar\'e duality, the exact cohomology sequence of the pair $(\bar{X}, \partial X)$ therefore becomes 
...
\textit{The current status of the problems and conjectures listed in} \cite{art4-key21}.
...
\bibitem{art4-key1} \textsc{A. Andreotti} and \textsc{H. Grauert}:  Algebraische K\"orper von automorphen Funktionen, \textit{G\"ottinger Nachrichten}, (1961), 39-48.

\bibitem{art4-key2} \textsc{W. Baily} and \textsc{A. Borel}: Compactification of arithmetic quotients of bounded symmetric domains, \textit{Ann. of Math.,} 84 (1966), 442-528.

\bibitem{art4-key3} \textsc{E. Bombieri}: The pluricanonical map of a complex surface, Several Complex Variables, I, \textit{Lecture Notes in Mathematics}, 155, Springer, (1970).

\bibitem{art4-key4} \textsc{A. Borel}: \textit{Linear Algebraic Groups,} Benjamin, (1969).

\bibitem{art4-key5} \textsc{A. Borel}: Some metric properties of arithmetic quotients of symmetric spaces and an extension theorem, \textit{J. Diff: Geom.,} 6 (1972), 543-560.

\bibitem{art4-key6} \textsc{E. Cattani}: \textit{On the partial compactification of the ``modular variety'' associated to a K\"ahler surface}: Thesis, St. Louis, (1972) .

\bibitem{art4-key7} \textsc{S. S. Chern}: A generalization of K\"ahler geometry, \textit{Symposium in honor of S. Lefschetz}: Princeton Univ. Press. (1957). 103-121,

\bibitem{art4-key8} \textsc{H. Clements}: Picard-Lefschetz theorem for families of nonsingular algebraic varieties acquiring ordinary singularities, \textit{Trans. Amer. Math. Soc.,} 136 (1969), 93-108.

\bibitem{art4-key9} \textsc{H. Clemens}\pageoriginale and \textsc{P. Griffiths}: The intermediate Jacobian of the cubic threefold, \textit{Ann. of Math.,} 96 (1972), 281-356.

\bibitem{art4-key10} \textsc{H. Clements} and \textsc{W. Schmid}: Degeneration of algebraic manifolds, to appear.

\bibitem{art4-key11} \textsc{P. Deligne}: Le travaux de Griffiths,  \textit{S\'eminaire Bourbaki} Exp. 376.

\bibitem{art4-key12} \textsc{P. Deligne}: Th\'eorie de Hodge I, \textit{Actes du Congr\`es International des Mathematiciens,} Nice, (1970).

\bibitem{art4-key13} \textsc{P. Deligne}: The\'eorie de Hodge II, \textit{Publ. Math. I.H.E.S.,} 40(1972), 5-57.

\bibitem{art4-key14} \textsc{P. Deligne}: The\'eorie de Hodge III, to appear in \textit{Publ. Math. I.H.E.S.} 

\bibitem{art4-key15} \textsc{P. Deligne}: Equations diff\'erentielles \`a points singuliers r\'eguliers, \textit{Lecture Notes in Mathematics,} 163, Springer, (1970).

\bibitem{art4-key16} \textsc{V. Gugenheim} and \textsc{D. C. Spencer}: Chain homotopy and the Poincar\'e lemma, \textit{Proc. Amer. Math. Soc.,} 7 (1956), 144-152.

\bibitem{art4-key17} \textsc{R. Godement}: Topologie algebriques et theoric des faisceaux, \textit{Publ. Inst. Math. Univ. Strabourg}, XIII, Paris, Hermann, (1958).

\bibitem{art4-key18} \textsc{P. Griffiths}: Periods of integrals on algebraic manifolds I, II, \textit{Amer. J. Math.,} 90 (1968), 568-626, 805-865.

\bibitem{art4-key19} \textsc{P. Griffiths}: Periods of integrals on algebraic manifolds III, \textit{Publ. Math. I.H.E.S.}, 38 (1970), 125-180.

\bibitem{art4-key20} \textsc{P. Griffiths}: On the periods of certain rational integrals, I, II, \textit{Ann. of Math.}, 90 (1969) , 460-541.

\bibitem{art4-key21} \textsc{P. Griffiths}: Periods of integrals on algebraic manifolds: summary of main results and discussion of open problems, \text{Bull. Amer. Math. Soc.,} 76 (1970), 228-296.

\bibitem{art4-key22} \textsc{P. Griffiths}: Some transcendental methods in the study of algebraic cycles, Proceedings of the Maryland Conference on Complex Analysis, \textit{Lecture Notes in Mathematics}, 185, Springer, (1971).

\bibitem{art4-key23} \textsc{P. Griffiths}: Complex analytic properties of certain Zariski open sets on algebraic varieties, \textit{Ann. of Math.,} 94 (1971), 21-51.

\bibitem{art4-key24} \textsc{P. Griffiths}: and \textsc{J. King}: Nevanlinna theory and holomorphic mappings between algebraic varieties, to appear in \textit{Acta Math.,} 130 (1973).

\bibitem{art4-key25} \textsc{P. Griffiths}:\pageoriginale and \textsc{W. Schmid}: Locally homogeneous complex manifolds, \textit{Acta Math.}, 123 (1969), 253-302.

\bibitem{art4-key26} \textsc{A. Grothendieck}: On the de Rham cohomology of algebraic varieties, \textit{Publ. Math. I.H.E.S.}, 29 (1966), 95-103. 

\bibitem{art4-key27} \textsc{S. Helgason}: \textit{Differential Geometry and Symmetric Spaces}, Academic Press, New York, (1962) .

\bibitem{art4-key28} \textsc{E. Hille}: \textit{Analytic Function Theory, vol, II}, Ginn and Company, New York, (1962). 

\bibitem{art4-key29} \textsc{H. Hironaka}: Resolution of singularities of an algebraic variety over a field of characteristic zero I, II, \textit{Ann. of Math.,} 79 (1964), 109-326.

\bibitem{art4-key30} \textsc{W. V. D. Hodge}: \textit{The Theory and Application of Harmonic Integrals}, 2nd ed., Cambridge Univ. Press, (1952).

\bibitem{art4-key31} \textsc{N. Katz}: Nilpotent connections and the monodromy theorem; application of a result of Turritin, \textit{Publ. Math. I.H.E.S.,} 39 (1971), 175-232.

\bibitem{art4-key32} \textsc{P. Kiernan} and \textsc{S. Kobayashi}: Satake compactification and extension of holomorphic mappings, to appear in \textit{Inventiones Math}.

\bibitem{art4-key33} \textsc{K. Kodaira} and \textsc{D. C. Spencer}: On deformations of complex analytic structures III, \textit{Ann. of Math.,} 71 (1960).

\bibitem{art4-key34} \textsc{S. Kobayashi}: \textit{Hyperbolic Manifolds and Holomorphic Mappings}: Marcel Dekker, New York, (1970).

\bibitem{art4-key35} \textsc{A. Landman}: On the Picard-Lefschetz transformations, to appear in \textit{Trans. Amer. Math. Soc.}

\bibitem{art4-key36} \textsc{D. Lieberman}: Intermediate Jacobians, \textit{Proc. Oslo Conf.,} (1970).

\bibitem{art4-key37} \textsc{A. Mayer}: Families of $K3$ surfaces, \textit{Nagoga J. Math.,} 48 (1972).

\bibitem{art4-key38} \textsc{R. Nevanlinna}: \textit{Analytic Functions, Springer}, (1970).

\bibitem{art4-key39} \textsc{H. Nickerson}: On the complex form of the Poincar\'e lemma, \textit{Proc. Amer. Math. Soc.,} 9 (1958), 183-188.

\bibitem{art4-key40} \textsc{I. I. Piatetskii-Shapiro} and \textsc{I. R. Shafarevich}: Theorem of Torrelli on algebraic surfaces of type $K3$, \textit{Izvestia Akad. Nauk,} 35 (1971).

\bibitem{art4-key41} \textsc{W. Schmid}: Variation of Hodge structure---the singularities of the period mapping, to appear in \textit{Inventiones Math}.

\bibitem{art4-key42} \textsc{A. N. Tjurin}:\pageoriginale The geometry of the Fano surface of a nonsingular cubic $F \subset P^4$ and Torelli's theorem for Fano surfaces and cubics. \textit{Izvestia Akad. Nauk,} 35 (1971), 498-529.

\bibitem{art4-key43} \textsc{A. N. Tjurin}: Five lectures on three-dimensional varieties, \textit{Uspeki Mat. Nauk,} 27 (1972), 4-50.

\bibitem{art4-key44} \textsc{F. Warner}: \textit{Foundations of differentiable manifolds and Lie groups}: Scott, Foresman and Co., (1971).

\bibitem{art4-key45} \textsc{A. Weil}: Vari\'et\'es K\"ahl\'eriennes, \textit{Publ. Inst. Math. Univ. Nancago,} VI, Hermann, Paris (1958).

\bibitem{art4-key46} \textsc{A. Weil}: Un th\'eor\`eme fondamental de Chern en g\'eome\'etric riemannienne, \textit{S\'eminaire Bourbaki}, Exp. 239.

\bibitem{art4-key47} \textsc{R. Wells}: \textit{Differential analysis on complex manifolds}, to appear.

\bibitem{art4-key48} \textsc{H. Wu}: Normal families of holomorphic mappings, \textit{Acta. Math.,} 119 (1967), 193-233.
\end{thebibliography}
