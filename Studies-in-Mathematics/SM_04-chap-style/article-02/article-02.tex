...
\item[(iii)] Let $S=\Spec k$, where $k$ is a field of characteristic not 2, $U=\Spec k[x]$, and let $R=\Delta \Perp \Gamma\to U\times U$, where $\Delta$ is the diagonal, and where $\Gamma$ is the complement of the origin in the anti-diagonal
$$
\Gamma=\{(x,-x)|x\neq 0\}.
$$
...
\item[\rm(ii)] let $\phi:U'\to U$ be etale and surjective. Then $f\in P$ if and only if $f\phi\in P$.
\end{itemize}
\end{definition}
