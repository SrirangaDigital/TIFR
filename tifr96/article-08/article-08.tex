...
\begin{equation}
\delta : \bfT_{\lambda}(\Delta)\to H^{1}(V,\mathscr{O}(\Hom(\bfE_{\lambda},\bfE_{\lambda}))).\label{art08-sec6-eq6.3}
\end{equation}
...
\begin{equation}
\vcenter{\xymatrix{
 & H^{1}(V,\mathscr{O}(\Hom(\bfE,\bfE)))\ar@{--}[dd]^{\zeta}\\
\bfT_{0}(\Delta)\ar[ur]^-{\delta}\ar[dr]_-{\phi_{*}} & \\
 & H^{q}(V,\Omega^{q-1})
}}\label{art08-sec6-eq6.4}
\end{equation}
What we want is $\zeta:H^{1}(V,\mathscr{O}(\Hom(\bfE,\bfE)))\to H^{q}(V,\Omega^{q-1})$ which will always complete \eqref{art08-sec6-eq6.4} to a commutative diagram.
...
\begin{equation}
\zeta(\eta)=qP_{q}(\underbrace{\Theta,\ldots,\Theta}_{q-1},\eta)\quad (\eta\in H^{1}(V,\mathscr{O}(\Hom(\bfE,\bfE)))),\label{art08-sec6-eq6.8}
\end{equation}
