...
\end{equation}
where $\zeta_{2}=0$ is a suitable local equation for $K$ and $\zeta_{1}=0$ for the line passing through the point and along which $V^{(2)}_{a}$ is branched over $W$.\pageoriginale The singularity of type \eqref{art11-sec4-eq5} can be resolved according to \cite{art11-key6}, \S\ref{art11-sec3.4}, where
\begin{equation}
